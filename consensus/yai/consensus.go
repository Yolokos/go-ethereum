...
type POUWBeacon struct {
	ethone consensus.Engine // Original consensus engine used in eth1, e.g. ethash or clique
}

// New creates a consensus engine with the given embedded eth1 engine.
func New(ethone consensus.Engine) *POUWBeacon {
	if _, ok := ethone.(*POUWBeacon); ok {
		panic("nested consensus engine")
	}
	return &POUWBeacon{ethone: ethone}
}

// Author implements consensus.Engine, returning the verified author of the block.
func (beacon *POUWBeacon) Author(header *types.Header) (common.Address, error) {
...
func (beacon *POUWBeacon) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header) error {
...
func (beacon *POUWBeacon) splitHeaders(headers []*types.Header) ([]*types.Header, []*types.Header) {
	var (
		preHeaders  = headers
...
func (beacon *POUWBeacon) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header) (chan<- struct{}, <-chan error) {
...
func (beacon *POUWBeacon) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (beacon *POUWBeacon) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header) error {
...
func (beacon *POUWBeacon) verifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, ancestor *types.Header) (chan<- struct{}, <-chan error) {
	var (
		abort   = make(chan struct{})
		results = make(chan error, len(headers))
	)
...
func (beacon *POUWBeacon) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
...
func (beacon *POUWBeacon) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state vm.StateDB, body *types.Body) {
	if !beacon.IsPoSHeader(header) {
...
func (beacon *POUWBeacon) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, body *types.Body, receipts []*types.Receipt) (*types.Block, error) {
	if !beacon.IsPoSHeader(header) {
...
func (beacon *POUWBeacon) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	if !beacon.IsPoSHeader(block.Header()) {
...
func (beacon *POUWBeacon) SealHash(header *types.Header) common.Hash {
...
func (beacon *POUWBeacon) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
func (beacon *POUWBeacon) Close() error {
...
func (beacon *POUWBeacon) IsPoSHeader(header *types.Header) bool {
